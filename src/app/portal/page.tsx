...
type Enrollment = { id: string; service_type: string; status: string; progress: number; created_at: string; };
...
export default function PortalDashboard() {
  ...
  enrollments.forEach((e) => {
    ...
    if (e.status === "pending" && ageHours > 48) {
      attentionItems.push({ icon: meta.icon, msg: `${meta.label} review is overdue (${Math.round(ageHours / 24)}d)`, tone: "red", href: `/portal/intake?service=${e.service_type}` });
    } else if (e.status === "active" && e.progress < 30) {
      attentionItems.push({ icon: meta.icon, msg: `${meta.label} needs more info (${e.progress}%)`, tone: "gold", href: `/portal/intake?service=${e.service_type}` });
      ...
      attentionItems.push({ icon: meta.icon, msg: `${meta.label} pending review`, tone: "gold", href: `/portal/intake?service=${e.service_type}` });
    }
  });
  ...
  return (
    <div className="space-y-5">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-3">
          {serviceCards.map((s) => (
            <Link key={s.id} href={`/portal/intake?service=${s.id}`} className="no-underline">
              <Card className="cursor-pointer !p-4 hover:-translate-y-0.5 transition-transform h-full" style={{ borderTop: `3px solid ${s.color}` }}>
                ...
                {!s.enrollment && (
                  <button className="mt-3 w-full text-[11px] font-bold text-[#0B4DA2] hover:underline">Get Started →</button>
                )}
              </Card>
            </Link>
          ))}
