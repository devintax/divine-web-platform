...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!res.ok) {
      return NextResponse.json({ error: json.error || json.message || "Login failed" }, { status: res.status });
    }
    ...
    const { data: profile } = await admin.from("user_profiles").select("id,role,legal_name,email").eq("auth_user_id", userId).single();
    if (profile) {
