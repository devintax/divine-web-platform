...
async function main() {
  ...
  const intake = await request("/api/services/enroll", client.auth_user_id, {
    ...
    body: JSON.stringify({
      ...
      intakeData: {
        ...
        taxYear: "2025",
        incomeTypes: ["W-2"],
        notes: "Automated E2E lifecycle probe",
      },
    }),
  });
  const enrollmentId = intake.enrollmentId;
  if (!enrollmentId) throw new Error("Enrollment was not created");

  await request("/api/workflows/tax", client.auth_user_id, {
    method: "POST",
    body: JSON.stringify({ enrollmentId, clientEmail: client.email, clientName: "E2E Client", userId: client.id }),
  }).catch((error) => {
    console.warn("[e2e] workflow start skipped:", error.message);
  });

  const cases = await request("/api/admin/cases?service=tax", staff.auth_user_id);
  if (!cases.cases?.some((c: any) => c.id === enrollmentId)) throw new Error("Submitted tax case did not appear in staff queue");

  const missing = await request(`/api/cases/${enrollmentId}/missing-docs`, staff.auth_user_id, {
    method: "POST",
    body: JSON.stringify({ documentName: "W-2", instructions: "Upload the latest W-2 for E2E verification." }),
  });
  ...
  docForm.append("files", new Blob(["E2E W-2 placeholder"], { type: "application/pdf" }), "w2-e2e.pdf");
  ...
  await request(`/api/cases/${enrollmentId}/messages`, staff.auth_user_id, {
    method: "POST",
    body: JSON.stringify({ message: "Your documents are received. We are preparing the draft return." }),
  });
  await request(`/api/cases/${enrollmentId}/messages`, client.auth_user_id, {
    method: "POST",
    body: JSON.stringify({ message: "Thank you. Please proceed with the draft." }),
  });

  const deliverableForm = new FormData();
  deliverableForm.append("title", "E2E Draft Tax Return");
  deliverableForm.append("description", "Automated draft return for lifecycle verification.");
  deliverableForm.append("requiresApproval", "true");
  deliverableForm.append("file", new Blob(["E2E tax return placeholder"], { type: "application/pdf" }), "draft-tax-return.pdf");
  ...
  await request(`/api/cases/${enrollmentId}`, staff.auth_user_id, {
    method: "PATCH",
  ...
  if (enrollment?.status !== "completed" || enrollment?.progress !== 100) {
    throw new Error("Case did not complete correctly");
  }
