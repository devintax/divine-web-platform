        ...
        <div className="bg-white border border-border rounded-[20px] p-8 shadow-sm">
          {success ? (
            <div className="text-center py-4">
              ...
              <p className="text-sm text-muted leading-relaxed mb-6">We&apos;ve sent a 6-digit verification code to <strong className="text-ink">{email}</strong>.</p>
              ...
              <form onSubmit={handleVerifyEmail} className="mx-auto max-w-[280px] space-y-4 text-left">
                <div>
                  <label className="text-xs font-bold text-muted block mb-1.5">Verification Code</label>
                  <input
                    type="text"
                    inputMode="numeric"
                    value={code}
                ...
                <Btn variant="primary" type="submit" disabled={verifyLoading || code.length !== 6} className="w-full">
                  {verifyLoading ? "Verifying..." : "Verify Email"}
                </Btn>
              </form>
              <div className="mt-5">
                <Link href="/login" className="text-sm text-[#0B4DA2] font-bold no-underline hover:underline">Go to Login</Link>
              </div>
            </div>
          ) : (
            <>
